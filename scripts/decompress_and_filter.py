...
def run_filter(RAW='tweet_data', CLEANED='filteredtweets', CLEAN_FN='filtered.json', COMP_FN='complete.txt', ERROR_FN='error.txt'):
    '''
    takes compressed json files, decompresses and filters tweets,
    then appends tweets to json files.

    Parameters
    ----------
    RAW : folder to data, structured as <rawfolder>/<date>/<file>.jsonl.gz

    CLEANED : foldername for cleaned data json files

    CLEAN_FN : filename to save filtered tweets as

    COMP_FN : filename for text list of completed files

    ERROR_FN : filename for text list of files that could not be processed
    '''
    ...
    def main(RAW, CLEANED, CLEAN_FN, COMP_FN, ERROR_FN):
        ...
        tot_files = len([file
                         ...
                         for file in os.listdir(RAW + folder)])  # total files to be filtered through
        ...
        try:
            ...
            print('Directory {0} already exists, continuing with designated directory.'.format(
                CLEANED))
            pass
        ...
        for FOLDER in os.listdir(RAW):
            for FILENAME in os.listdir(RAW + FOLDER):  # get filenames

                R_FILENAME = RAW + FOLDER + '/' + FILENAME  # add directory to filename
                ...
                if not check_fn(COMP_FN, R_FILENAME):
                    ...
                    if (cmp_files % 10 == 0) & (cmp_files != 0):  # print update
                        ...
                        print(
                            'There are {0} tweets found with geo locations so far.'.format(tot_tweets))
        print('Found a total of {0} tweets.'.format(tot_tweets))
        print(
            'Successfully opened {0}/{1} files from {2}'.format(cmp_files, tot_files, RAW))

    main(RAW, CLEANED, CLEAN_FN, COMP_FN, ERROR_FN)
